package com.rz51.core.shiro.session;
...
import com.rz51.core.shiro.CustomShiroSessionExt;
...
import com.rz51.user.enti.UserOnlineExt;
...
public class CustomSessionManager {
	 ...
	 * session status 
	 */
	public static final String SESSION_STATUS ="rz51-online-status";
	ShiroSessionRepository shiroSessionRepository;
	
	CustomShiroSessionExt customShiroSessionExt;
	
	/**
	 * 获取所有的有效Session用户
	 * @return
	 */
	public  List<UserOnlineExt> getAllUser() {
		//获取所有session
		Collection<Session> sessions = customShiroSessionExt.getActiveSessions();
		List<UserOnlineExt> list = new ArrayList<UserOnlineExt>();
		
		for (Session session : sessions) {
			UserOnlineExt bo = getSessionBo(session);
			if(null != bo){
				list.add(bo);
			}
		}
	 ...
	 * @return
	 */
	public UserOnlineExt getSession(String sessionId) {
		Session session = shiroSessionRepository.getSession(sessionId);
		UserOnlineExt bo = getSessionBo(session);
		return bo;
	}
	private UserOnlineExt getSessionBo(Session session){
		//获取session登录信息。
		Object obj = session.getAttribute(DefaultSubjectContext.PRINCIPALS_SESSION_KEY);
		if(null == obj){
			return null;
		...
		if(obj instanceof SimplePrincipalCollection){
			SimplePrincipalCollection spc = (SimplePrincipalCollection)obj;
			/**
			 * 获取用户登录的，@link AuthRealm.doGetAuthenticationInfo(...)方法中
			 * return new SimpleAuthenticationInfo(user,user.getPswd(), getName());的user 对象。
			 */
			obj = spc.getPrimaryPrincipal();
			if(null != obj && obj instanceof UUser){
				//存储session + user 综合信息
				UserOnlineExt userBo = new UserOnlineExt((UUser)obj);
				//最后一次和系统交互的时间
	...
	public Map<String, Object> changeSessionStatus(Boolean status,
		...
		try {
			...
			for (String id : sessionIdArray) {
				...
				customShiroSessionExt.update(session);
			}
			map.put("status", 200);
	...
	public void forbidUserById(Long id, Long status) {
		//获取所有在线用户
		for(UserOnlineExt bo : getAllUser()){
	...
	public void setCustomShiroSessionExt(CustomShiroSessionExt customShiroSessionExt) {
		this.customShiroSessionExt = customShiroSessionExt;
	}
}
